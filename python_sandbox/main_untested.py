...
class FeatureExtractor:
    ...
    def extract(self, smiles_list):
        ...
        for smiles in tqdm(smiles_list):
            ...
            # Extended RDKit descriptors with error handling
            try:
                desc = [
                ...
                logger.warning(f"Descriptor calculation failed for SMILES {smiles}: {e}")
                desc = [0.0] * 60
            ...
            feat = np.clip(feat, -1e6, 1e6)  # Clip extreme values
...
# Use the existing production PolymerDataset instead of the basic implementation
# The ProductionPolymerDataset is imported above and provides better functionality
...
def test_dataloader_functionality():
    ...
    logger.info("Testing DataLoader functionality with sample data...")
    ...
    sample_data = {
        ...
        'Tg': [1.0, 2.0, None, 3.0, 4.0],  # Some missing values
    ...
    logger.info("Checking array lengths in sample_data:")
    for key, value in sample_data.items():
        logger.info(f"  {key}: {len(value)} elements")
    
    sample_df = pd.DataFrame(sample_data)
    
    try:
        # Test dataset creation using production PolymerDataset
        dataset = ProductionPolymerDataset(sample_df, target_cols=config.TARGET_COLS, is_test=False)
        ...
        for i in range(len(dataset)):
            data = dataset[i]
            if data is not None:
                logger.info(f"Sample {i}: {data.x.shape[0]} atoms, {data.edge_index.shape[1]} edges")
                if hasattr(data, 'y'):
                    logger.info(f"  Targets: {data.y.tolist()}")
                    logger.info(f"  Mask: {data.mask.tolist()}")
            else:
                logger.info(f"Sample {i}: Failed to process (likely invalid SMILES)")
        
        # Test DataLoader with proper collate function
        def create_safe_dataloader(dataset, batch_size, shuffle=False):
            """Create a DataLoader with proper error handling for invalid SMILES."""
            from torch.utils.data import DataLoader as TorchDataLoader
        ...
        for batch_idx, batch in enumerate(loader):
            if batch is not None:
                logger.info(f"Batch {batch_idx}: {batch.x.shape[0]} total atoms, batch size: {batch.batch.max().item() + 1}")
                if hasattr(batch, 'y'):
                    logger.info(f"  Batch targets shape: {batch.y.shape}")
                    logger.info(f"  Batch mask shape: {batch.mask.shape}")
            else:
                logger.info(f"Batch {batch_idx}: Empty batch (all samples failed)")
        
        # Test with GCN model
        logger.info("Testing with GCN model...")
        # Get number of atom features from a sample
        sample_data = None
        for data in dataset:
            if data is not None:
        ...
        logger.info("DataLoader functionality test completed successfully!")
        return True
...
def test_with_real_data():
    """Test the DataLoader with actual CSV data files."""
    logger.info("Testing DataLoader with real data files...")
    
    try:
        # Update config to use local data path
        config.DATA_PATH = 'info'
        
        # Load a small subset of real data for testing
        train_df = pd.read_csv(f'{config.DATA_PATH}/train.csv')
        test_df = pd.read_csv(f'{config.DATA_PATH}/test.csv')
        ...
        logger.info(f"Loaded train subset: {train_subset.shape}, test subset: {test_subset.shape}")
        
        # Test dataset creation with real data using production PolymerDataset
        dataset = ProductionPolymerDataset(train_subset, target_cols=config.TARGET_COLS, is_test=False)
        test_dataset = ProductionPolymerDataset(test_subset, target_cols=config.TARGET_COLS, is_test=True)
        
        logger.info(f"Created datasets: train={len(dataset)}, test={len(test_dataset)}")
        ...
        loader = create_safe_dataloader(dataset, batch_size=3, shuffle=False)
        test_loader = create_safe_dataloader(test_dataset, batch_size=3, shuffle=False)
        
        # Test training data batching
        logger.info("Testing training data batching...")
        valid_batches = 0
        for batch_idx, batch in enumerate(loader):
            if batch is not None:
                valid_batches += 1
                logger.info(f"Train batch {batch_idx}: {batch.x.shape[0]} atoms, batch size: {batch.batch.max().item() + 1}")
                if hasattr(batch, 'y') and hasattr(batch, 'mask'):
                    logger.info(f"  Targets shape: {batch.y.shape}, Mask shape: {batch.mask.shape}")
        
        logger.info(f"Processed {valid_batches} valid training batches")
        
        # Test test data batching
        logger.info("Testing test data batching...")
        valid_test_batches = 0
        for batch_idx, batch in enumerate(test_loader):
            if batch is not None:
                valid_test_batches += 1
                logger.info(f"Test batch {batch_idx}: {batch.x.shape[0]} atoms, batch size: {batch.batch.max().item() + 1}")
        
        logger.info(f"Processed {valid_test_batches} valid test batches")
        
        # Test with GCN model on real data
        logger.info("Testing GCN model with real data...")
        # Get number of atom features from a sample
        sample_data = None
        for data in dataset:
            if data is not None:
        ...
        for batch in loader:
            if batch is not None:
                with torch.no_grad():
                    output = model(batch)
                logger.info(f"GCN output shape: {output.shape}")
                logger.info(f"Sample predictions: {output[0].tolist()}")
                break
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Polymer Prediction Pipeline')
    parser.add_argument('--test', action='store_true', help='Run tests instead of main pipeline')
    parser.add_argument('--test-real', action='store_true', help='Test with real data')
    args = parser.parse_args()
    
    if args.test:
        # Run test functions
        logger.info("Running test mode...")
        if test_dataloader_functionality():
            logger.info("Sample data test passed!")
            
            if args.test_real:
                if test_with_real_data():
                    logger.info("All DataLoader tests passed!")
                else:
